...
class Phr(implicit p: Parameters) extends PhrModule with HasPhrParameters with Helpers {
  ...
  private def getRedirectPhr(phrMeta: PhrMeta): UInt = {
    ...
    Cat(redirectErrorPhr(PhrHistoryLength - 1, PathHashHighWidth), phrMeta.hashHigh)
  }
  ...
  private val s0_phrRegValue = getPhr(RegEnable(s0_phrPtr, !s0_stall))
  ...
  s1_overrideData.phrMeta.hashHigh := s1_phrValue(PathHashHighWidth - 1, 0)
  ...
  when(updateData.valid) {
    ...
    for (i <- 1 to PathHashHighWidth) {
      phr((updateData.phrMeta.phrPtr + i.U).value) := updateData.phrMeta.hashHigh(i - 1)
    }
    when(updateData.taken) {
      ...
      for (i <- 1 to PathHashHighWidth) {
        phr((updateData.phrMeta.phrPtr + i.U).value) := hashHigh(i - 1) ^ updateData.phrMeta.hashHigh(i - 1)
      }
  ...
  io.phrMeta.hashHigh      := s1_phrValue(PathHashHighWidth - 1, 0)
  io.phrMeta.predictionPhr := s1_phrValue
  io.phr                   := phr
  ...
  private val redirectHistDiff = redirectData.valid &&
    (redirectData.phrMeta.predictionPhr(MaxHistLens - 1, 0) =/= redirectPhr(MaxHistLens - 1, 0))
  XSPerfAccumulate("histFoldedPhr_diff_s0_foldedPhrReg", histFolded_diff_s0Folded)
  XSPerfAccumulate("redirect_hist_diff", redirectHistDiff)
