...
//   private val hits = VecInit(predictStateTouchs.map(_.map(_.valid).reduce(_ || _)))
  ...
  dontTouch(predictSetIndexVec)
  dontTouch(predictStateEntries)
  dontTouch(predictNextState)
  ...
  dontTouch(trainSetIdx)
  dontTouch(trainStateEntry)
  dontTouch(trainNextState)
  dontTouch(trainVictimWay)
  dontTouch(trainTouchWay)
